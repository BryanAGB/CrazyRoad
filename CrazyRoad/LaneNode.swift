...
class LaneNode: SCNNode {
        ...
        for index in 0..<Int(width) {
            if type == .grass {
                if randomBool(odds: 7) {
                    let vegitagion = getVegitation()
                    vegitagion.position = SCNVector3(x:10 - Float(index), y:0 , z:0)
                    laneNode.addChildNode(vegitagion)
                }
            }
            else if type == .road {
                carGap += 1
    ...
    func getVegitation() -> SCNNode {
        ...
        let vegitation = randomBool(odds: 2) ? Models.tree.clone() : Models.hedge.clone()
        return vegitation
    }
